...
def convert_lead_to_customer(lead_id, company):
	# Avoid duplicates during live conversion
...
def get_site_logo():
	...
	if logo:
		return {"logo_url": frappe.utils.get_url(logo)}
...
def sync_client(payload):
	...
	try:
		...
		# Check if Lead already exists by phone to prevent sync duplicates
		if phone:
...
def sync_visit(payload):
	...
	try:
		...
		# Resolve Salesperson User ID to active Sales Person DocType Link
		sales_person = frappe.db.get_value("Sales Person", {"employee": frappe.session.user}, "name") or frappe.session.user
...
def sync_order(doc_type, payload):
	...
	try:
		doctype_found, docname_found = resolve_customer_or_lead(data.get("customer"))
		if not doctype_found:
			return {"success": False, "error": f"Customer/Lead '{data.get('customer')}' not found."}
